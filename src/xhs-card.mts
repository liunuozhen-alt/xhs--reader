...
export default async (req: Request) => {
  ...
  try {
    ...
    if (!shareUrl && !noteId) return json({ ok: false, error: '缺少参数' })

    let note = null
    try { note = await fetchDirect(noteId, shareUrl, commentCount) } catch {}
    const apiKey = getTikHubApiKey()
    if (!note && apiKey) {
      try { note = await fetchTikHubImage(noteId, shareUrl || shareText) } catch {}
    }
    if (!note) return json({ ok: false, error: '无法获取笔记' })

    const rid = note.noteId || noteId
    if (rid && fetchCommentFlag && apiKey && (!note.comments || note.comments.length === 0)) {
      try {
    ...
    if (note.type === 'video' && !note.videoUrl && rid && apiKey) {
      try { note.videoUrl = await fetchTikHubVideo(rid) } catch {}
    }
    return json({ ok: true, note, source: note._source || 'unknown' })
  } catch (e: any) {
